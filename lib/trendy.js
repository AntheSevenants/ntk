...
// The old parsing remains available as well
...
const constituentCodeModeSwitcher = document.getElementById("constituentCodeModeSwitcher");
constituentCodeModeSwitcher.onchange = () => { setTranslationFunctions(); };
...
function setTranslationFunctions() {
	if (constituentCodeModeSwitcher.checked) {
...
function translate2019()
{
	...
	translateTimeout = window.setTimeout(function()
	{
		...
		for (let level = 0; level < codeLevels.length; level++)
		{
			...
			// Separated by spaces
			let codes = codeLevels[level].split(" ");
...
function createConstituent(words, constituentFunction)
{
	...
	if (typeof constituentCodes[constituentCodeMode][constituentFunction] != "undefined")
	{
